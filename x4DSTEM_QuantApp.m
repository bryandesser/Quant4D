classdef x4DSTEM_QuantApp < handle
    %
    % Usage:
    %     app.x4DSTEM_QuantApp
    ...
    properties (Constant)
        AppPathBase = {'/4DSTEM_Quant'};
        AppClass = 'x4DSTEM_QuantApp';
        Increment = 1;
        Decrement = 0;
        Version = '13a';
    end
    ...
    methods
        ...
        function startApp(obj)
            ...
            mlock;
            x4DSTEM_QuantApp.refcount(obj.Increment);
            ...
            existVal = exist(fullfile(pwd,'DPC_Quant')); %#ok<EXIST>
            ...
            if (doesShadowExist && ~pathOne.equals(pathTwo))
                % We are trying to execute the wrong DPC_Quant
                errordlg(message('MATLAB:apps:runapp:WrongEntryPoint', 'DPC_Quant').getString, ...
            ...
            if nargout(@DPC_Quant) == 0
                eval('DPC_Quant');
            else
                obj.AppHandle = eval('DPC_Quant');
            end

...
%Main Function
function outputData = dm4Reader(inName,writeTagsOnly)
	...
	if(writeTagsGlobal)
		[FIDout, FIDoutmessage] = fopen(strcat(inName,'_tags.txt'),'wt');
        if(FIDout == -1)
	...
	%DM4 files have an unnamed root group which contains everything in the file
    ...
    if(writeTagsGlobal)
        %disp('Binary data not read into memory. All tags werre written to a text file.')
        ...
        if numObjects == 1
            ...
            if(~isempty(scaleUnit) && length(scaleUnit{1}) >= 0)
                outputData.scaleUnit = scaleUnit;
                outputData.scale = scale;
            end
            ...
            for ii = 2:numObjects
                if( (xsize(ii) > 0))
                    ...
                    elseif strcmp(stype,'complex16')
                        ...
                        elseif zsize2(ii)>1 %data is a 4D array
...
function output = validDM4(FID)
    ...
    %The first 3 4byte integers have header information
    ...
    %unknown part of the DM4 header
    fread(FID,1,'uint32','ieee-be'); 
...
% 	OCTET   = 10;
%   ULONG64 = 12
%   is string, array or struct = -1
    switch encodedType
		case {0} %just in case...
			width = 0;
		case {8, 9, 10}		%{BOOLEAN, CHAR, OCTET}
			width = 1; %data is 1 byte each
		case {2, 4}			%{SHORT, USHORT}
			width = 2;
		case {3, 5, 6}		%{LONG, ULONG, FLOAT}
			width = 4;
		case {7, 12} %{DOUBLE}
			width = 8;
		otherwise
...
function val = readNativeData(FID,encodedType)
    ...
    elseif(encodedType == 12);
        val = fread(FID,1,'uint64'); %Not sure what 12 should be. Maybe signed int64?
	else
		error(['readNativeData: Unknown data type ' num2str(encodedType)])
        val = -1
	end
